...
<div class="column">
        <?php if(!empty($_SESSION['islogin'])) : ?>
            <?php
            $sql = $pdo->prepare("SELECT * FROM product WHERE provider_id = ? && is_active = 1 ;");
                    ...
                    <div class="box" style="margin: 25px; display: flex; align-items: center;">
                        <div style="flex-grow: 1;">
                            ...
                            <p><strong>参加人数:</strong>
                                <?= htmlspecialchars($product['current_participants']) ?>/<?= htmlspecialchars($product['max_participants']) ?>人
                            </p>
                        </div>

...
<?php
    if( isset( $_GET['purchase_id'] ) )
    {
        ...
        if( $getPurchases )
        {
            ...
            $count_sql->execute([ $getPurchases['product_id'], $getPurchases['start_time'] ]);

            foreach( $count_sql as $attendance )
            {
                $attendanceSet = $pdo->prepare(" UPDATE dates SET purchased_num = ? WHERE product_id = ? AND start_time = ? ; ");
                $attendanceSet->execute([ $attendance['total'], $getPurchases['product_id'], $getPurchases['start_time'] ]);
            }
        }
